...
void test_circ_buf_init_should_succeed(void) {
    int ret = circ_buf_init(&cb, storage, sizeof(storage)/sizeof(buffer_type), sizeof(buffer_type));
...
void test_circ_buf_init_should_fail_on_invalid_args(void) {
    TEST_ASSERT_EQUAL_INT(-1, circ_buf_init(NULL, storage, sizeof(storage)/sizeof(buffer_type), sizeof(buffer_type)));
    TEST_ASSERT_EQUAL_INT(-1, circ_buf_init(&cb, NULL, sizeof(storage)/sizeof(buffer_type), sizeof(buffer_type)));
    TEST_ASSERT_EQUAL_INT(-1, circ_buf_init(&cb, storage, 0, sizeof(buffer_type)));
}

// ------------------- PUSH TESTS -------------------

void test_circ_buf_push_should_store_value(void) {
    circ_buf_init(&cb, storage, sizeof(storage)/sizeof(buffer_type), sizeof(buffer_type));
